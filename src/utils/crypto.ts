...
export async function importKey(jwk: JsonWebKey): Promise<CryptoKey> {
  return window.crypto.subtle.importKey('jwk', jwk, { name: ALGO_NAME }, true, ['encrypt', 'decrypt']);
}

export async function encryptMessage(msg: string, key: CryptoKey): Promise<string> {
